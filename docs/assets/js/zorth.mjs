...
["_malloc","__emscripten_thread_init","__emscripten_thread_exit","__emscripten_thread_crashed","__emscripten_thread_mailbox_await","__emscripten_tls_init","_pthread_self","checkMailbox","establishStackSpace","invokeEntryPoint","PThread","___indirect_function_table","__emscripten_proxy_main","___set_stack_limits","_main","onRuntimeInitialized"].forEach((prop) => {
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'zorth.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = new URL('zorth.wasm', import.meta.url).href;
}
...
if (ENVIRONMENT_IS_PTHREAD) { assert(Module['wasmOffsetData'], 'wasmOffsetData not found on Module object'); }
if (ENVIRONMENT_IS_PTHREAD) { wasmOffsetConverter = resetPrototype(WasmOffsetConverter, Module['wasmOffsetData']); }
...
// end include: preamble.js
  ...
  allocateUnusedWorker() {
        ...
        if (!Module['locateFile']) {
          worker = new Worker(new URL('zorth.worker.mjs', import.meta.url), {type: 'module'});
        } else {
        // Allow HTML module to configure the location where the 'worker.js' file will be loaded from,
        // via Module.locateFile() function. If not specified, then the default URL 'worker.js' relative
        // to the main html file is loaded.
        var pthreadMainJs = locateFile('zorth.worker.mjs');
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(3, 0, 1, dirfd, path, flags, varargs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(5, 0, 1, status);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 0, 0, atomicIndex);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 0, 1, fd,offset_low, offset_high,whence,newOffset);
...
var __emscripten_tls_init = Module['__emscripten_tls_init'] = createExportWrapper('_emscripten_tls_init', 0);
...
var unexportedSymbols = [
  ...
  'PTY_pollTimeout',
  'PTY_askToWaitAgain',
  'PTY_waitForReadableWithCallback',
  'PTY_handleSleepWithAtomic',
  'PTY_waitForReadableWithAtomicImpl',
  'PTY_atomicIndex',
  'PTY_waitForReadableWithAtomic',
  'PTY_handleSleep',
  'PTY_waitForReadable',
];
